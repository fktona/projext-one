...
use crate::screenpipe::{fetch_screenpipe_data_with_query_and_offset, fetch_screenpipe_data_with_offset};
...
pub fn emit_progress(app_handle: &tauri::AppHandle, current: usize, total: usize, status: &str) {
    let progress = if total > 0 { (current as f64 / total as f64) * 100.0 } else { 0.0 };
...
pub async fn perform_data_export(app_handle: &tauri::AppHandle, custom_query: Option<serde_json::Value>) -> Result<(), String> {
    let desktop_path = get_desktop_path()?;
    let export_dir = desktop_path.join("project one test");
    ...
    println!("Starting export from offset: {}, batch: {}", export_state.last_offset, export_state.batch_number);
    ...
    if export_state.is_first_run || export_data.metadata.total_items > export_state.total_items_known as usize {
    ...
    let filename = format!("screenpipe_export_batch_{}_{}.json", export_state.batch_number, timestamp);
    ...
    if export_progress.current_offset >= export_progress.total_items {
        ...
        emit_progress(app_handle, 100, 100, "Export completed! Starting new batch...");
    } else {
        emit_progress(app_handle, 100, 100, "Batch completed! More data available...");
    }
    ...
    println!("Progress: {}/{} items exported (offset: {})", 
        export_progress.items_exported, 
        export_progress.total_items, 
        export_progress.current_offset
    );
    Ok(())
}
